  ...
  <nav class="bg-white border-gray-200 dark:bg-gray-900">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
      <a href="https://flowbite.com/" class="flex items-center space-x-3 rtl:space-x-reverse">
        ...
        <span class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">Chic Tech</span>
      </a>
      <div v-if="canLogin" class="flex items-center md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
        <div class="mr-3">
          <button type="button" class="relative inline-flex items-center p-3 text-sm font-medium text-center">
            ...
            <div
              class="absolute inline-flex items-center justify-center size-6 text-xs font-bold text-white bg-red-500 border-2 border-white rounded-full -top-1 -end-1 dark:border-gray-900">
              20</div>
          </button>
        </div>

        <button v-if="auth.user" type="button"
          class="flex text-s rounded-full md:me-0 focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600"
          id="user-menu-button" aria-expanded="false" data-dropdown-toggle="user-dropdown"
          data-dropdown-placement="bottom">
          <span class="sr-only">Abrir menu</span>
          <svg class="size-8 rounded-full bg-tranparent" xmlns="http://www.w3.org/2000/svg" fill="none"
        ...
        <div v-else>
          <Link :href="route('login')" v-if="canLogin" type="button"
